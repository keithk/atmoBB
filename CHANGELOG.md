...
- Keith owes, before this reaches a real forum:
  ...
  - [ ] Publish the extension kit (`docs/extensions/kit`) to npm once outside authors are invited; first fix its relative import into `src/lib/extensions/contract`, which only resolves inside this repository, and the absolute `file:` dependency `new` writes into a scaffolded project.
