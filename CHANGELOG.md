...
- Admin → Updates on an installation without the host updater now explains which installs can update from the page and points everything else at the upgrade docs, instead of telling every install to rerun `./atmobb install`.
- Rollout, for operators: the `app.atmobb.extension.binding` and `app.atmobb.extension.endorsement` lexicons are published from the authority account; set `ADDRESS_HEADER`/`XFF_DEPTH` on any deployment behind a proxy so signed-out extension actions rate-limit per visitor instead of per proxy; the atmobb.app directory forum, and only it, sets `ATMOBB_EXTENSION_DIRECTORY=1` and `ATMOBB_EXTENSION_DIRECTORY_DID`; and after upgrading, a forum that installs an extension reconnects its forum account from Admin → Extensions once the PDS's permission-set cache (about 10 minutes) has cleared. The authoring kit and its dice example live under `docs/extensions/` and stay out of the image.
