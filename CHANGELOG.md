...
- Schemas, all additive: `app.atmobb.forum.stamp` and `app.atmobb.forum.getStamps` are new; `app.atmobb.forum.membership` gains optional `wearing`; `app.atmobb.moderation.action` gains the `awardStamp` and `revokeStamp` kinds and optional `actor`; `app.atmobb.forum.profile` gains optional `hideDefaultStamps` and deprecates `ranks`, which is no longer read. `getMembers`, `getMembership`, `getThreadPage`, and `moderation.getLog` gain optional output fields, and their post-count fields are deprecated. `app.atmobb.authForum` is unchanged, so nobody re-consents.
